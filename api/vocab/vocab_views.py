...
from api.models import VocabFSRS, Word, NotebookWord
...
class VocabSyncView(APIView):
    ...
    批量同步用户词汇：新词建空卡，旧词仅更新中文；
    返回全部卡片状态及统计。线程安全：select_for_update + bulk_create(ignore_conflicts=True)
    """
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                # 第二次尝试：如果指定的 plan_id 不存在，检查是否存在该词的其他计划版本。
                # 这处理了前端 plan_id 错误或单词被添加到多个计划的情况。
