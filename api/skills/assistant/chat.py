...
def skill_assistant_react_browser_system_prompt(
    ...
    prompt = (
        ...
        '== 文件浏览工具 (仅可读源码; .env/密钥类文件已屏蔽) ==\n'
        '{"action":"list_frontend_dir","path":"src/components","reason":"查看前端代码结构"}\n'
        '{"action":"read_frontend_file","path":"src/App.tsx","reason":"读取代码内容以分析"}\n\n'
        '== 用户数据查询工具 (只读，只涉及当前登录用户自身) ==\n'
        '{"action":"query_user_stats","reason":"获取当前用户的综合学习统计"}\n'
        '  → 返回: 学习计划数、笔记本数、词书进度、AT币余额、注册时间等\n'
        '{"action":"query_user_preferences","reason":"查看用户目标与偏好设置"}\n'
        '  → 返回: 各项目标分数、考试日期、UI 语言、AI 提供商、词书难度等\n'
        '{"action":"query_vocab_plans","reason":"查看用户的学习计划详情"}\n'
        '  → 返回: 所有学习计划名称、每日学词数、掌握目标等\n'
        '{"action":"query_notebooks","reason":"查看用户的笔记本列表"}\n'
        '  → 返回: 笔记本名称和每本包含的词条数量\n'
        '{"action":"query_fsrs_due","reason":"查看当前到期需复习的单词数量"}\n'
        '  → 返回: 当前到期、24小时内、7天内到期的词数\n'
        '{"action":"search_words","keyword":"abandon","reason":"在用户词库中搜索某个词"}\n'
        '  → 返回: 匹配到的单词及其释义、所在笔记本\n'
        '{"action":"list_todos","reason":"查看用户的待办事项"}\n'
        '  → 返回: 待办 id/文字/是否完成/创建日期，最多 50 条\n'
        '{"action":"list_shortcuts","reason":"查看用户的快捷链接"}\n'
        '  → 返回: 快捷 id/标题/URL/是否新窗口打开，最多 20 条\n\n'
        '== 用户操作工具 (可以真的改数据; 只影响当前用户自己) ==\n'
        '{"action":"add_todo","text":"复习昨天的错题","reason":"根据用户请求新增待办"}\n'
        '  → 效果: 创建一条待办，返回 changed.created_todo_id\n'
        '{"action":"toggle_todo","todo_id":12,"reason":"用户表示已完成该待办"}\n'
        '  → 效果: 切换 done 状态\n'
        '{"action":"delete_todo","todo_id":12,"reason":"用户不再需要"}\n'
        '  → 效果: 删除该待办\n'
        '{"action":"add_shortcut","title":"每日练习","url":"/practice","open_in_new_tab":false,"reason":"用户请求加快捷入口"}\n'
        '  → 效果: 创建快捷链接; url 必须以 http(s):// 或 / 开头\n'
        '{"action":"delete_shortcut","shortcut_id":3,"reason":"清理"}\n'
        '  → 效果: 删除该快捷\n'
        ...
        '5) 执行任何"写"类工具 (add_/toggle_/delete_/update_) 前，必须已经从用户消息里明确读到该意图；'
        '不要主动替用户猜或修改数据。若不确定，务必先在 final_answer 里确认。\n'
        + final_lang_rule
    )
